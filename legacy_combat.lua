...
function CommonOutgoingHitSuccess(e)
	...
	off_weapon = self:GetInventory():GetItem(Slot.Secondary);
	...
	if (handcheck == 14) then
		...
		baneweapontype		 = off_weapon:GetItem():BaneDmgBody();
	...
	if (e.hit.skill == 28 and self:GetClass() == Class.MONK and weapondamage == 0) then
	...
	if (e.self:IsClient() and e.other:IsNPC()) then
		...
		eq.debug("C to NPC");
		eq.debug("Is offhand attack " .. handcheck);
		...
		if (e.hit.skill == 8) then
			...
			if self:GetSTR() > 200 then
				backstab_str = (((self:GetSTR() - 200) / 5) + 200);
			else
				backstab_str = self:GetSTR();
			end

			backstab_attackmod = self:GetATK() / 5;
			backstab_mod = 2 + (self:GetSkill(Skill.Backstab) * 0.02 );
			backstab_min = (((weapondamage * backstab_mod) * backstab_str) / 100) + backstab_attackmod;
			backstab_max = (((self:GetSkill(Skill.Offense) + backstab_str) * (weapondamage * backstab_mod)) / 70); 
			backstab_damage = Random.Real(backstab_min, backstab_max);

			defenderac = defenderac * .10;

			final_damage = backstab_damage - defenderac;

			eq.debug("backstabdamage " .. backstab_damage);
		...
		if (e.hit.skill == 51) then
			bowmindmg = 1;
			bowmaxdmg = (rangedweapondamage * offensivemod);
			...
			final_damage = final_damage - mitigationac;
		end
	
		--bow damage
		if (e.hit.skill == 7) then
			bowmindmg = 1;
			bowmaxdmg = ((rangedweapondamage + ammoweapondamage) * offensivemod);
		...
		if (e.hit.skill == 38) then
			...
			final_damage = roundkickdmg - mitigationac;
		end
		...
		if (e.hit.skill == 52) then
			...
			final_damage = tigerdmg - mitigationac;
		end
		...
		if (e.hit.skill == 26) then
			...
			final_damage = Random.Real(flyingmindmg, flyingmaxdmg);
		...
		if (e.hit.skill == 0 or e.hit.skill == 1 or e.hit.skill == 28 or (e.hit.skill == 36 and itemtype ~= 35)) then --1her
			if (handcheck == 13) then
				...
				final_damage = (damage_roll + attack_roll + str_roll) - mitigationac;

				eq.debug("Primary Weapon damage roll before discs / dmg caps " .. final_damage);
			end

			if (handcheck == 14) then
				minimumdamage = 1;
				maximumdamage = ((weapondamage * offensivemod));
				...
				eq.debug("Secondary Weapon damage roll before discs / dmg caps " .. final_damage);
			end
		end
		...
		if e.hit.skill == 2 or e.hit.skill == 3 or (e.hit.skill == 36 and itemtype == 35) then --2her
			...
			local damage_roll = Random.Real(minimumdamage, maximumdamage);
			local attack_roll = Random.Real(0, (playeratkbonus * 2));
			local str_roll = Random.Real(0, (strbonus * 2));
	...
	if (e.self:IsClient() and e.other:IsClient()) then
		...
		if e.hit.skill == 8 then
			...
			backstab_ac = defenderac / 75;

			final_damage = backstab_damage - backstab_ac;

			if (other:CastToClient():IsSitting()) then
				final_damage = backstab_max - backstab_ac;
			end
		end
		...
		if e.hit.skill == 51 then
			...
			final_damage = (damage_roll + attack_roll + dex_roll) - mitigationac;

			if (other:CastToClient():IsSitting()) then
				final_damage = bowmaxdmg - mitigationac;
			end
		end
		...
		if e.hit.skill == 38 then
			...
			if (other:CastToClient():IsSitting()) then
				final_damage = roundkickmaxdmg - mitigationac;
			end
		end
		...
		if e.hit.skill == 52 then
			...
			if (other:CastToClient():IsSitting()) then
				final_damage = tigermaxdmg - mitigationac;
			end
		end
		...
		if e.hit.skill == 26 then
			...
			final_damage = flyingdmg - mitigationac;

			if (other:CastToClient():IsSitting()) then
				final_damage = flyingmaxdmg - mitigationac;
			end
		end
		...
		if (e.hit.skill == 0 or e.hit.skill == 1 or e.hit.skill == 28 or (e.hit.skill == 36 and itemtype ~= 35)) then --1her
			...
			if (other:CastToClient():IsSitting()) then
				final_damage = (maximumdamage + playeratkbonus + strbonus) - mitigationac;
			end
		end
	...
	--FINAL CHECKS BEFORE APPLYING DAMAGE, BANE, DISC, LEVEL CAP, MITIGATION, AND CRITICAL.

	if ((e.other:GetBodyType() == baneweapontype) and e.other:IsNPC()) then
		final_damage = final_damage + baneweapondamage;
	end

	if ((e.other:GetRace() == baneweaponrace) and e.other:IsNPC()) then
		final_damage = final_damage + baneweaponracedamage;
	end
	...
	if (other:FindBuff(4510)) then
		final_damage = final_damage * .40;
	end
	if (other:FindBuff(4499)) then
		final_damage = final_damage * .65;
	end
